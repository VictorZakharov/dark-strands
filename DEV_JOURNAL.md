...
## 2026-07-17 (round 15 — ez-tree triangle budget fix)

- **ez-tree vegetation triangle budget cut ~2.31M → ~1.18M main-pass tris** (review finding verified by independent measurement — all 10 preset counts reproduced exactly with ez-tree 1.1.0). New `CFG.EZTREE.DETAIL` per-category caps clamp each preset's `branch.sections`/`branch.segments` per recursion level before `generate()` (branch TUBES were the dominant cost — raw Bush 3: 13.7k branch tris for a 1u shrub, now 3.4k) and replace the global `LEAF_MULT` with per-category `leafMult` whose leaf size is compensated by 1/sqrt(mult) to preserve canopy coverage (`ezTreeFactory.js`).
- **Pass-level trims**: bushes now cast leaves-only sun shadows (branch tubes invisible inside their own shadow blob; shadow maps render every frame on WebGPU) and are excluded from the water-mirror render list via new `mesh.metadata.ezCategory` tag (`terrainMeshes.js`) — old card system never reflected bushes either. Aggregate vegetation vertex work across main+shadow+fog+mirror passes ≈ 9.2M → ≈ 4.2M tris/frame.

## 2026-07-17 (round 16 — ez-tree vegetation landed: real trees/bushes, autumn palette, forest zoning, demo grass/flowers/ambience)

- **ez-tree integration (core, via multi-agent workflow)**: card-based leafy trees, fir cone-tiers and bush blobs fully replaced by real generated geometry from `@dgreenheck/ez-tree` 1.1.0 (MIT) + three.js, bundled from package src via esbuild (`lib/eztree.bundle.js`, ~4.1MB with textures embedded as data URIs; `npm run bundle:eztree`; importmap 'eztree', dynamic import at world build). New `src/world/ezTreeFactory.js` converts each preset's branches/leaves BufferGeometry verbatim into a Babylon template pair (both engines RH Y-up — no winding flip) and thin-instances all placements: 10 variants (Oak/Ash/Aspen/Pine/Bush presets), 20 draw calls total. Bark = color+normal data-URI textures; leaves = the proven alpha-test card recipe + preset tint. Old proxy/card/cone construction and its texture generators deleted; pine grove clustering, grid blocking, trunk physics, `treePosData` damping all preserved. THIS CLOSES THE TABLED FIR REDESIGN — firs are now ez-tree pines.
- **Per-instance autumn palette** (`CFG.EZTREE.TINTS` + thin-instance 'color' buffer on leaves): weighted per-category tints — leafy: green/yellow-green/golden/orange, pine: subtle green shifts, bush: green + odd yellowing — with 0.85-1.1 brightness jitter. Channels >1 legally brighten in-shader, which is what turns green-leaning leaf textures vivid gold/orange (verified live on WebGPU before implementing).
- **Zoned planting** (`vegetation.js` placeTrees rewrite): BFS road-distance field over the walk grid drives three passes — single-species FOREST stands (2 pine + 2 leafy zone centers ≥ 12 cells from roads, ≥ 22 apart, uniform-disc fill with 1-cell spacing), ROADSIDE ornamental leafy trees (road-dist 2-3 cells, avenue spacing), and lone SCATTER (`CFG.EZTREE.ZONING`). TREES 80 → 140, BUSHES 90 → 100; bushes now seed 55% around forest-edge rings / 20% road verges / 25% anywhere.
- **Demo-app assets (MIT, ship inside the npm package)**: grass tufts replaced by the ez-tree demo's `grass.glb` blade-clump (12 verts) thin-instanced ~9k times with meadow-green instance tints — up-normals reapplied (twoSidedLighting FLIPS hand-set up-normals on back-winding faces → near-black tufts; culling-off without twoSidedLighting is the correct recipe, same as the old tufts). White/yellow/blue wildflower GLBs scattered on grassy cells (1 draw call per color). `assets/sounds/ambience.mp3` looping ambient (new `systems/audio.js`, HTMLAudioElement, starts on Enter World via sticky activation, pauses with the sim).
- **Fixes**: floating/notched trunk bases (ez-tree trunks are open-bottom tubes; SINK 0.1 → 0.35 below the MIN of 5 sampled nearby terrain heights — coarse ground lattice dips proved 0.1 insufficient); boulder-through-tree (placeRocks now rejects cells within 1 ring of a tree cell — rocks place after trees); bush-in-boulder (placeBushes rejects `collidesWithRock`); frozen lake invisible in snow biome (iceMat 0.722,0.831,0.890 was near-identical to the snow ground under the bloom/fog pipeline — now blue-glossy 0.55,0.72,0.86 with hard specular + cold emissive, plus isPickable=false).
- Verified in browser across two worlds (normal + snow): zero pageerrors, 130 trees/100 bushes/9k grass/140 flowers placed, ambience playing, frozen lake clearly reads as ice, forests/roadside/scatter all present, autumn oaks vivid. Foreground FPS 50-107 across shots.

## 2026-07-17 (round 17 — wind sway, snow vegetation tints, plaster reflections, glass shard rework)

- **Wind sway vertex animation** (new `src/world/windSway.js`): a `MaterialPluginBase` subclass injects a 3-octave sine displacement at `CUSTOM_VERTEX_UPDATE_WORLDPOS` — post-instance-transform so the wind blows in a coherent world direction regardless of each thin instance's yaw, with phase seeded from world position (spatial hash stands in for the ez-tree demo's simplex3). Ships BOTH GLSL and WGSL `getCustomCode` variants (StandardMaterial/PBR compile native WGSL on WebGPU in Babylon 9.17) and overrides `isCompatible()` (base class rejects WGSL). Sway weight per mesh type: ez-tree leaves by `uv.y` (0 at branch attachment, 1 at tip — their only base/tip signal), grass + wildflowers by local `position.y` (grass uv.v is INVERTED — uv weighting would wave the roots; flower petal-atlas UVs carry no height). Attached to leaf materials per category (`ezTreeFactory.js`), grass tuft material, and every flower sub-material (`vegetation.js`); driven from the main loop via `updateWindSway(gdt, getWeatherModifiers())` — weather already publishes windX/windZ (CLEAR 1.5 → STORM 10-12), time integrates gdt (pauses with sim, honors Q fast-forward, wrapped at 2π×4096 so f32 never loses sin precision), strength `min(1, 0.18 + mag*0.082)`. `CFG.GFX.WIND_SWAY` + `CFG.WIND` amp/freq tunables; runtime kill-switch `window._gfx.wind(false)`. Amplitudes kept ≤0.25 on trees: sun-shadow + fog-depth passes use their own shaders (verified in Babylon source — no plugin hooks), so their silhouettes don't sway. KEY DESIGN (from adversarial review): per-material amp/invHeight ride the material UBO (bound in `bindForSubMesh`), NOT baked as shader literals — Babylon's global effect cache keys on shader-name+defines, so two leaf categories with identical defines would otherwise share one compiled effect and every leaf type would inherit whichever amp compiled first. Weight-mode (uv vs height) is a `#define` (`WINDSWAYUV`) so height-mode materials never reference `uvUpdated` (only in scope with UV1).
- `lib/babylon-entry.js`: + `MaterialPluginBase` (non-pure path — runs the RegisterMaterialPluginBase side effect), + `Engines/Extensions/engine.query` and `AbstractEngine/abstractEngine.timeQuery` side-effect imports — the PERF instrumentation's `captureGPUFrameTime` was a missing prototype method on the WebGL2 engine, so `?webgl` worlds failed to build (pre-existing since the 9.17 bundle trim; WebGPU has the method natively, masking it).
- **Snow-biome vegetation tints** (`CFG.EZTREE.TINTS_SNOW` + branch at the single tint-read in `spawnEz`): frosted palettes for pine (deep green / cold blue-green / frosted pale / heavy rime) and bush (cold dull green / frosted / rimed white-blue, red damped vs blue — bush presets carry warm material tints underneath) with tighter 0.9-1.05 jitter (bloom blows out near-white at the autumn palette's 1.1 top). Instance tints only MULTIPLY the green-leaning leaf textures (can't add missing blue), so snow also swaps the leaf material emissive to a cool blue-grey (0.09,0.12,0.18) — the icy cast is additive. Branch-at-read keeps a later non-snow world from inheriting snow palettes.
- **Plaster/timber water reflections**: `wallsPlaster` + `wallTrims` added to the water-mirror name allowlist in `terrainMeshes.js` (plaster houses reflected as bare stone; timber trim didn't reflect at all). Stale walls.js comment updated.
- **Glass shard rework** (`windows.js` spawnGlassShards): the old digest source was an UNSUBDIVIDED plane — `CreatePlane` has no `subdivisions` option, so every break spawned just 4 pane-sized triangles sharing the rock's direction (the "shards don't spread" bug), in per-FRAME velocity units, with the sheet unrotated on E/W walls. Now: 40 thin-box shards (0.17×0.22×0.02, per-shard scale jitter) via `sps.addShape`, scattered across the true pane rect (tryBreakWindow passes isNS + real pane dimensions), velocity = rock-carry + radial burst from the impact point in the pane plane + up-bias, all dt-scaled (u/s, clamped 0.1s), random tumble rates, per-shard terrain-height landing (was a fixed y=-0.5 world plane), holds during sim freeze (`scene.animationsEnabled` gate), `isPickable=false`. Review fixes: a `p.landed` flag stops the per-particle `getTerrainHeight` sample once a shard settles (was re-sampling all 40 every frame for the full 3s), and disposal now calls only `sps.dispose()` (it disposes its own `buildMesh()` output — the prior `mesh.dispose()` + `sps.dispose()` double-disposed the same mesh).
- Verified live on WebGPU: sway pixel-diff (grass region 7.8 wind-on vs 2.0 wind-off, canopy 8.0 vs 3.1, static controls low; `_gfx.wind(false)` kills it), shard burst visible mid-flight through a doorway (spread ✓ tumble ✓ thin boxes ✓), snow world frosted pine/bush mix + cool cast, mirror renderList contains wallsPlaster+wallTrims, waterRefl 15 draws (+1). WebGL2 (`?webgl`): world builds again, all compiled effects ready, autumn scene renders. `npm run build` green (CI parity).

## 2026-07-17 (round 18 — open-source readiness: MIT license, third-party notices, README as tech demo)

- **License audit → cleared for MIT.** Read every dependency's actual LICENSE from `node_modules` (authoritative, not guessed): `@babylonjs/core`/`loaders`/`materials` = Apache-2.0, `@babylonjs/havok` = **MIT** (the make-or-break item — Babylon.js publishes the Havok WASM wrapper under MIT), `@dgreenheck/ez-tree` / `three` / `esbuild` = MIT. Committed assets: Poly Haven textures (grass/stone_wall/bark/wood_planks/fabric) = CC0; Soldier/Flower.glb + ez-tree demo models/`grass.jpg`/`ambience.mp3` = MIT; `Fox.glb` = CC-BY 4.0. Every component is permissive and MIT-compatible — nothing GPL/proprietary/non-commercial, nothing needs replacing. Only hard attribution obligation is the CC-BY-4.0 fox.
- **New `LICENSE`** — MIT, © 2026 Victor Zakharov, with an appended clarifying note that MIT covers the original source only and third-party deps/assets retain their own licenses (see notices file).
- **New `THIRD-PARTY-NOTICES.md`** — consolidates every runtime library and every committed art/audio asset with version, license, copyright, and source URL. Fox attribution quoted verbatim from Khronos glTF-Sample-Assets (model by PixelMannen CC0 / rig+animation by @tomkranis CC-BY 4.0 / glTF conversion by @AsoboStudio + @scurest CC-BY 4.0 — confirmed against the upstream README). Doubles as the Apache-2.0 NOTICE-retention + MIT license-text-retention record for the deployed bundles.
...
## 2026-07-17 (round 22 — ez-tree pickable wildflowers, per-color hotbar, tree bark lighting)

- **The wildflower field is now the flower pickup/planting system — every flower is pickable/plantable.** Replaced the crystalline `Flower.glb` pickup with the white/yellow/blue ez-tree demo wildflowers (`assets/models/eztree/flower_*.glb`) that were already scattered decoratively. `world/flowers.js` now drives pick/plant directly on the thin-instance matrix buffers built by `vegetation.js placeFlowers`, so the whole field is interactive in ~3 draw calls (no mesh per flower). Picking hides an instance (parked far below the world via a zero-scale matrix) and frees its slot; planting fills a free slot from a reserved `PLANT_HEADROOM` (45/color) pool. Removed the old individual-mesh flower path (the `MODEL_REGISTRY` flower entries + `modelLoader.js` flower template/clone/registration code).
- **Per-color hotbar slots + color-matched icons.** Each flower color is its own hotbar item type (`flower_white`/`flower_yellow`/`flower_blue`) with its own inventory count (`flowerWhite`/`flowerYellow`/`flowerBlue`) and a colored 5-petal SVG icon (`hud.js` `flowerSVG`, `hotbar.js` `ITEM_META` + drag placeholders). Different colors land in different slots (existing `addItemToSlot` dedup); planting uses the SELECTED slot's color (`controls.js` passes the item type to `plantFlower`; `main.js` passes `slotItem` to `updateFlowerPreview`, and gates placement on `slotItem.startsWith('flower_')`).
- **No respawn.** Picked flowers stay gone — their slot returns to the plant pool. Removed the respawn system (`updateFlowers`, `randomFlowerPos`, `projectToScreen`); the field no longer repopulates itself. Slots are conserved (pick frees one, plant fills one) so planting never runs dry.
- **Placement rules tightened.** Can't plant on roads (`isRoadCell`) or on building floors — added `generator.isOnBuildingFloor(wx, wz, margin)` (full footprint + 0.6u margin) which, unlike `isInsideBuilding` (interior shrunk 1u), also rejects the wood floor at the doorway threshold. Retained the water/walkable/rock/range checks. Ghost goes red on any invalid spot.
- **Fixed the plant-mode freeze.** The translucent placement ghost is now a cheap `StandardMaterial` clone (not the heavy field PBR material), and its pipeline is prewarmed during load — parked in front of the camera for the world-build warm-up `scene.render()`, then hidden (`prewarmFlowerPreviews`/`hideFlowerPreview`, same idiom as the held-torch/rain prewarms) — so the first plant-mode toggle no longer stalls compiling the shader.
- **Tree "hollow"/dark fix** (folded into this branch). Trunks read as near-black hollows because ez-tree's bark tube rendered single-sided with the OUTER wall culled — you were seeing the dark INNER wall (inner faces point away from the sun, hence the darkness). Fixed by rendering bark double-sided with two-sided lighting (`ezTreeFactory.js`: `backFaceCulling=false` + `twoSidedLighting=true`, the same recipe the leaves use) so the correctly-lit outer surface always shows and the open trunk bottom is no longer see-through. Also lifted the bark diffuse toward a woody brown (`Color3.Lerp` 0.4) and added an `emissiveColor` ambient floor (0.1) for canopy-shadowed trunks (the GlowLayer is include-only/torches, so bark doesn't bloom), and bumped `CFG.EZTREE.SINK` 0.35→0.6 to bury the open trunk bottom. (An earlier emissive-only pass missed because the wrong surface was showing; the sidedness was the real bug.) Perf note: double-siding doubles bark rasterization — if it costs frame time, flipping the branch winding is the perf-free alternative.
- **Removed unused `assets/models/Flower.glb`** and its README / CLAUDE.md / THIRD-PARTY-NOTICES.md references (the crystal flower is no longer used anywhere).
- Reviewed the rewrite with an adversarial multi-agent pass; the single confirmed finding — planting could silently no-op once the per-color headroom was exhausted under the old respawn model — is moot now that picks free slots and there is no respawn.

## 2026-07-18 (round 23 — fix mobile shadow desync: second writer of the sun light)

- **Fixed mobile-only shadow flicker, shadow lag while walking, and shadows appearing on sunlit faces.** Root cause: `entities/player.js` rewrote `sunLight.position` / `sunLight.direction` every frame (a CSM-era leftover — its own comment said "CSM handles this mostly"), duplicating and then fighting the authoritative writer, `updateSunShadow` in `core/lighting.js`. The two used different formulas (raw player position + sun offset vs. a texel-lattice-snapped position 70u back along the light direction), so they disagreed permanently.
- **Why it only showed on mobile.** The sun shadow map is throttled on the WebGL2 path only (`_throttleSunShadow = !isWebGPU()`, `lighting.js`): it re-renders on a 2-unit player snap, a sun-direction change, or an 8-frame heartbeat, and `updateSunShadow` early-returns on skipped frames *before* touching the light — deliberately, so the light stays exactly where the map was rendered from. But Babylon rebuilds the shadow sampling matrix from `sunLight.position` in the main pass **every** frame (`ShadowGenerator.bindShadowLight` → `getTransformMatrix`), so `player.js`'s unthrottled write on those skipped frames desynced the sampling matrix from the map contents — displacing the entire shadow field by ~1u at rest and ~3u while walking at mobile frame rates (~17 and ~50 shadow texels respectively). Desktop takes the WebGPU path, where the throttle is off, `updateSunShadow` overwrites the stray value every frame and the map re-renders every frame — hence desktop was unaffected.
- **Fix**: deleted the redundant write in `player.js` (and its now-unused `sunLight` parameter, `getSunOffset` import, and the `getSunLight()` argument at the `main.js` call site). Added a regression guard in `lighting.js`: the committed light position is cached whenever the map re-renders and re-asserted on every skipped frame, so any future second writer can't silently reintroduce the desync. No effect on the WebGPU path.
- Diagnosed with a multi-agent adversarial pass over the whole render pipeline. Notable **negative** results worth keeping: depth precision (`camera.minZ` 0.1 / `maxZ` 400) is *not* implicated — Babylon allocates sized `DEPTH_COMPONENT24` / `depth24plus`, so the driver has no latitude, and the sun map has its own `shadowMinZ`/`shadowMaxZ` range regardless. Also refuted: mediump shader precision, float/half-float RTT downgrade, MSAA sample mismatch, `setMaximumLimits`, mobile URL-bar resize storms, and thin-instance frustum strobe in portrait. Two tempting "fixes" that would regress: lowering `camera.maxZ` far-plane-clips the sky dome (diameter 600, sized against maxZ 400), and raising `camera.minZ` near-clips the first-person torch glow (and is a no-op anyway — `player.js` rewrites it every frame).
- **Still open**: there is no mobile graphics tier — `config.js` `GFX` is one flat profile and `scene.js` clamps DPR to 2 on every device, so phones render the full desktop chain (2048 PCF shadow map, half-float volumetric fog with a 24-tap god-ray march, full-res float depth RTT, every-frame planar water mirror, GlowLayer, clustered lights) at up to 2× device pixel ratio. Not addressed here; deliberately kept out so this fix can be attributed on its own.

## 2026-07-18 (round 24 — fix empty survival guide on mobile)

- **Fixed the survival guide rendering completely empty on mobile** (both orientations). Three things combined: the mobile media block is a *capability* query (`@media (hover: none) and (pointer: coarse)`, `styles.css`), so it applies in portrait and landscape alike; inside it `.help-tab-content[data-tab="controls"] { display: none }` hides the Controls tab content, winning over `.help-tab-content.active { display: flex }` on source order (both are specificity 0,2,0); and Controls is the only tab marked `active` in the markup. Mobile is supposed to switch to the Touch tab, but that happens in `setupHelpTabs()` (`systems/controls.js`), which was only ever called from `toggleHelp()` — the desktop SHIFT+? path. The mobile "?" button (`systems/touch.js`) flipped `style.display` directly, so no tab content was ever `.active` and the tab click handlers were never wired. `styles.css` also hides the Controls tab *button* with `!important`, so there was no recovery path either. Fix: export `setupHelpTabs` and call it from the mobile handler before showing the panel. Desktop is unchanged — its touch-tab auto-select is still gated on `'ontouchstart' in window`, and `panel._tabsReady` keeps the call idempotent.
- **The guide was also unusable even once visible.** `#help-overlay` was missing from the `touchstart` bypass list in `touch.js` (which already skips `#touch-buttons`, `#touch-top-buttons`, `#blocker`), so every touch on the open guide fell through to the joystick/look branches and got `preventDefault()`d — killing the tab buttons (click is synthesized from touch), killing scrolling, and walking the player around behind the overlay. Added the bypass, and mirrored the pause button's teardown when the guide opens (release joystick/look ids, zero movement, hide the long-press ring and joystick) since the bypass only blocks *new* touches and can't release an already-latched drag.
- **Revived a dead CSS block**: the mobile scroll/padding rule targeted `#help-content, #help-content-mobile` — neither ID has ever existed in `index.html`. Retargeted to `.help-tab-content` (the element that actually carries `overflow-y: auto`) and added `overscroll-behavior: contain`.
- Swept for sibling bugs with a multi-agent adversarial pass (16 of 29 findings survived verification). **Deferred, worth their own PRs**: the sleep menu (`#sleep-ui`) has the identical missing-bypass bug and is a genuine reload-only soft-lock on mobile (slider undraggable, buttons swallowed, `timeStopped` with no dismissable overlay); mobile help/pause don't freeze the sim or tear down placement mode (stuck flower/torch ghost over the pause overlay); help *content* has drifted (per-color wildflowers undocumented, PAUSE key missing, ESC behaviour described wrongly); and the JS/CSS "is mobile" predicates disagree (`'ontouchstart' in window` vs the `hover/pointer` media query), which diverges on touchscreen laptops.
- **Follow-up after device testing: the guide still wouldn't scroll.** The cause is `body { touch-action: none }` — the first rule inside the mobile media block. WebKit intersects `touch-action` across the *whole* ancestor chain (`none ∩ pan-y = none`), so the `pan-y` on `.help-tab-content` could never take effect; Chrome terminates the walk at the nearest scroll container, which is why this is iOS-shaped. That body rule can't simply be dropped: `index.html`'s viewport meta has no `user-scalable=no`, so it is the *only* pinch-zoom / double-tap-zoom guard during play. Fixed by gating it on state — `body.help-open { touch-action: pan-y }`, with the class toggled at all four help show/hide sites (`controls.js` toggleHelp + the ESC path, `touch.js` the "?" button + the mobile close button). `pan-y` rather than `auto` keeps zoom dead even while the guide is up. Rejected `body > *:not(#help-overlay)`: a multi-touch gesture resolves `touch-action` at the lowest common ancestor of its touch points, which for two thumbs on different children is `body` — that variant would silently re-enable pinch-zoom mid-game. Also added `html, body { overscroll-behavior: none }` outside the media query, since `overflow: hidden` does not stop Chrome Android pull-to-refresh and the property propagates to the viewport from the *root* element.
- **Mobile close control is now an X sized to the tabs.** Replaced the "Tap to close" `<span>` with a real `<button id="help-close-mobile" aria-label="Close">✕</button>` (semantic + screen-reader labelled; the existing `touchstart` handler is unchanged). Sized 40×40 as a comfortable tap target, with the Touch/World tabs pinned to a 28px height and switched to `inline-flex` centring so the row lines up. `#help-tabs` gets `margin-left: auto` in the mobile block, which consumes the header's free space so `space-between` has none left to spread — leaving title on the left and a tabs-then-gap-then-X cluster on the right. **`flex: 0 0 auto` on both the X and `#help-tabs` is load-bearing**: the header is a flex row and on a ~360-390px viewport the title + tabs + X overflow it, so a shrinkable item gets compressed horizontally and an explicitly-square button renders as a narrow rectangle (an explicit `width` does *not* prevent flex shrinking — only `flex-shrink: 0` does). `#help-title` gets `min-width: 0` so it absorbs the shortfall by wrapping instead.
- A flexbox theory (flex items' `min-height: auto` clipping the scroller) was investigated and **discarded**: per CSS Flexbox §4.5 the automatic minimum size is already 0 when computed `overflow` is not `visible`, and `.help-tab-content` has always had `overflow-y: auto`. The `min-height: 0` fix was a no-op and was reverted, which also means the earlier failed `pan-y` attempt was clean evidence rather than a confounded test.

## 2026-07-18 (round 26 — ?shadow= arm flag to settle WebGPU vs WebGL2)

- **Added `?shadow=throttle` / `?shadow=everyframe`** so the shadow-refresh strategy can be A/B'd against both backends from a single build. `core/scene.js` now owns the arm (`shadowThrottled()` / `shadowModeTag()`), resolved EAGERLY at the end of engine selection — `_useWebGPU` is only set after `await engine.initAsync()`, so a module-load-time read would see `false`. Defaults are bit-exact preserved when the param is absent (throttle on WebGL2, every-frame on WebGPU). `lighting.js` and `world/torchLighting.js` now both read `shadowThrottled()` instead of `!isWebGPU()`, so the flag covers the sun map **and** the 18 torch cube-face passes — the old `_gfx.shadowEveryFrame()` hook only touched the sun map and left `_throttleSunShadow` set, a half-state that read like a working toggle; it now just points at the URL param. HUD shows the arm (e.g. `FPS: 118 GPU/thr`) only when `?shadow=` is passed, since mobile has no console.
- **Why this matters: the original justification for every-frame WebGPU shadows may be void.** `lighting.js` documents a strobe when the shadow map is throttled on WebGPU, blamed on "a bit-exact replay of an earlier pass that ignores current uniforms". But *every one of those trials predates round 23*, which removed a second, unthrottled writer of `sunLight.position` in `player.js`. On skipped frames that stray write survived and Babylon rebuilt the sampling matrix from it — producing shadows that alternate **at exactly the refresh cadence**, verbatim the reported symptom. With a static sun and a stationary player the stray vector was *constant*, so the corrupted frames were pixel-identical between sessions, which is what the pixel-diff harness read as a "bit-exact replay". The headline evidence therefore never discriminated between stale-GPU-replay and matrix desync. If `?shadow=throttle` is clean on WebGPU, the ~870-draw sun pass plus 18 torch cube passes stop running every frame and the measured ~2x deficit (60-70 FPS vs up to 120 on `?webgl`) should largely close.
- **Also corrected**: the "bit-exact replay ignoring current uniforms" phrasing in `CLAUDE.md` / `lighting.js` describes Babylon's *snapshot rendering*, which is off by default and never touched anywhere in `src/`. Render bundles (non-compatibility mode) are ruled out as the mechanism too — they are not invalidated by frustum culling, and the strobe reproduced with compat mode both on and off. The symptom may be real but the stated mechanism should not be relied on. `scene.resetDrawCache()`, the documented escape hatch for non-compatibility-mode staleness, appears nowhere in the codebase and was never tried.
- **Separately diagnosed (not fixed here): the WebGPU rotation hitch.** Babylon's WebGPU backend calls the *blocking* `device.createRenderPipeline()` at draw time on a cache miss (`webgpuCacheRenderPipeline.js`), never the async variant it documents as preferred. WebGL2 cannot pay this — it has no pipeline state objects, and its path uses `KHR_parallel_shader_compile` to defer draws rather than block. The scene runs 7 render passes per frame, each with its own format/sample-count combination, so a newly-visible mesh compiles a separate pipeline *per pass it enters* — and `main.js`'s warm-up renders 5 frames all from spawn in one fixed orientation, so everything behind the player compiles on first sight. Likely fix is sweeping the warm-up camera through a yaw circle behind the loading screen; deferred until the arm test says whether WebGPU is staying.
- Investigated and rejected as ways to cut WebGPU shadow cost: shrinking `shadowFrustumSize` (Babylon 9 RTT render lists do **no** culling, so it saves zero draws and pops shadows inside the fog envelope), CSM (adds ~2600 draws and early-returns out of the throttle entirely), and halving `SHADOW_MAP` (doubles texel size, reopening the acne/eave-gap the low bias depends on). The only free win found is the dead lens-flare block on WebGL2 — five flares constructed with `null` textures and additive blending, contributing zero pixels for five wasted draws.
