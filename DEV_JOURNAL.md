...
## 2026-07-18 (round 25 — fix mobile sleep menu soft-lock)

- **Fixed a reload-only soft-lock: the sleep menu was completely dead to touch on mobile.** Same trap as the survival guide in round 24 — `#sleep-ui` was missing from the `touchstart` bypass list in `systems/touch.js`, so its touches fell through to the joystick/look branches and were `preventDefault()`d. That suppresses synthesized `click`, killing both buttons (`sleep.js` binds `click`) and the native range-thumb drag feeding the slider's `input` handler. Worse than the guide: `#sleep-ui` is `position:fixed; inset:0; z-index:55` and `#touch-controls` is `z-index:10`, so `document.elementFromPoint` returned `#sleep-ui` everywhere and even the pause/help buttons were unreachable — and `closeSleepMenu` (the only thing that clears `setTimeStopped(true)`) has no external callers, so world time stayed frozen with no escape. Added the bypass.
- **Extracted `releaseTouchDrags()`** in `touch.js` and called it from the sleep, pause and help paths. The bypasses only block *new* touches — they can't release an already-latched one, and the sleep menu opens *from* a right-side long press, so `lookId` is always still latched at that moment. Zeroing `lookDx`/`lookDy` is the load-bearing part: `touchmove` is **not** gated on `gameActive` and keeps accumulating look delta, while the only drain (`consumeTouchLook`) sits inside `main.js`'s frozen `isGameActive()` block — so dragging that thumb across the panel banked a whole gesture of yaw and replayed it as one camera whip on close. This also fixes the same latent whip on the pause and help paths, which previously reset movement but not the look accumulators.
- **Guarded the Pointer Lock calls in `sleep.js`** — a second, independent soft-lock. iOS Safari has no Pointer Lock API, and `openSleepMenu` called `document.exitPointerLock()` unguarded *after* `setTimeStopped(true)` but *before* `display = 'flex'`: a throw there froze world time with the panel never shown, which no touch fix could undo. `main.js` already wrapped its equivalent call; `sleep.js` was the outlier. Also hardened the confirm handler to run the sleep callback in a `finally`, since `closeSleepMenu()` runs first and a throw inside it used to swallow the sleep entirely (menu closed, time never advanced).
- **`touch-action` was investigated and deliberately left alone** — recorded so it isn't re-litigated. Round 24 needed `body.help-open { touch-action: pan-y }` only because `.help-tab-content` scrolls; `#sleep-panel` has no scroller. `touch-action` governs UA panning/zooming, not event dispatch, and does **not** block a native `<input type="range">` thumb drag — `none` is in fact the preferred value for drag widgets, since `auto`/`pan-y` lets the compositor claim the gesture and fire `pointercancel` mid-drag. Adding `body.sleep-open` would have relaxed the app's only pinch-zoom guard for zero benefit. **Zero CSS changes.** Likewise no `touchstart` listeners were added to the sleep buttons: once the bypass lands the real `click` fires (the viewport meta has `width=device-width`, so no 350ms delay), and a second path would double-fire the confirm callback.
- Note the sleep menu is only reachable with **Day/Night Cycle enabled** (`controls.js` gates the bed interaction on `isCycleEnabled()`), which is off by default — so this bug required that toggle to reproduce.
